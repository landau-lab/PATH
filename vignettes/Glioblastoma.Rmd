...
cells, by passing the GBM phylogeny `tree115` to the PATH function, `inv.tree.dist()`. 
...
Idf <- reshape2::melt(modxcor$Morans.I, 
                      value.name = "I")
...
We can then use the PATH function `PATH.inference()` to infer transition probabilities
...
Pinf <- PATH.inference(tree115, 
                       impute_branches = T, 
                       sample_rate_est = 10^-6)
...
xcgsea <- xcor_gsea(genes115, Winv, pathwaysH)
